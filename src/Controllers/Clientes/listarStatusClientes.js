...
const listarStatusClientes = async (req, res) => {
    ...
    try {
        ...
        if (status === 'Em dia' || status === 'Inadimplente') {
            clientes = await knex('clientes').where('status', status).leftJoin('usuarios', 'clientes.associado_id', 'usuarios.id')
                .select('clientes.*').where('usuarios.id', associado_id).limit(limite)
        } else {
